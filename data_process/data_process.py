...
class DataProcess:
    ...
    def get_needed_data(self):
        ...
        if self.previous_all_data.empty:
            logger.warning(f'{self.time}之前没有数据，无法计算')
            raise DataNotExistError(f'{self.time}之前没有数据，无法计算')
    ...
    def update_statistics_table(self):
        """更新统计表信息"""
        self.statistics_table.set_index(['coin_name', 'spider_web'], inplace=True)
        # 删除第一列
        self.statistics_table = self.statistics_table.drop(self.statistics_table.columns[0:1], axis=1)
