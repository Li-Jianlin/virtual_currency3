...
class MinuteFunctionHandler(FunctionHandler):
    ...
    def filter_and_update_data(self, to_be_filter_data: pd.DataFrame, record_data: pd.DataFrame, MAX_TIME_INTERVAL):
        ...
        filter_condition = ((filter_data['cnt'] == 1) | (filter_data['cnt'] <= 3 & (filter_data['coin_price'] < filter_data['first_price'])))
    ...
    def change_and_virtual_drop_and_price_func_1_minute(self):
        ...
        # AB虚降筛选
        change_and_virtual_drop_data_A_to_B = self.filter_by_virtual_drop(change_A_to_B_data, 'ge', AB_VIRTUAL_DROP)

        virtual_drop_and_change_data_A_to_B = change_and_virtual_drop_data_A_to_B.copy()

        # AB收盘价与C价格对比
        merged_ABC_data = virtual_drop_and_change_data_A_to_B.merge(C_data[['coin_name', 'spider_web', 'coin_price']],
